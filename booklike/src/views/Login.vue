...
const onSubmit = () => {
	AppAxios.get(`/users?email=${userData.email}`).then(users => {
		if (users.data.length < 1) return alert("⛔ Geçersiz email ya da şifre girdiniz! ⛔") //alert("Böyle bir kullanıcı yok!") //az bilgi ver
		...
		if (users.data[0].password !== derPassword) return alert("⛔ Geçersiz email ya da şifre girdiniz! ⛔") //alert("Yanlış şifre!") //az bilgi ver
		const userId = users.data[0].email.split('@')[0];
		commit("LOGIN_USER", { ...users.data[0], userId });
		const goto = router.currentRoute.value.redirectedFrom || { path: `/users/${userId}` };
	...
	<div class="w-full h-full flex items-center justify-center">
		<div class="w-[27.5rem] mx-auto px-12 py-14 bg-white shadow-lg rounded-md flex flex-col">
			...
			>
				...
				<span class="mr-auto">Google hesabı ile Giriş yap</span>
			</button>
				...
				<ul class="grid gap-y-8">
					...
					<li class="flex flex-col text-xs text-gray-500">
						...
						<div class="mt-2">
							...
							>Şifrenizi mi unuttunuz?</router-link>
						</div>
					</li>
					<li>
						<button
					...
					<li class="text-sm text-center text-gray-500">
						Henüz bir hesabınız mı yok?
						<router-link :to="{ name: 'RegisterPage' }" class="text-blue-400 hover:underline">Kayıt ol</router-link>
					</li>
				</ul>
			</form>
		</div>
